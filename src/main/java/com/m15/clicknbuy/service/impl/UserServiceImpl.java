...
//	}
	...
	@Override
	public String paymentSuccess(String paymentId, String orderId, String signature,
	        String address, Principal principal,
	        HttpSession session, ModelMap map) {
	    ...
	    try {
	        ...
	        Order order = new Order();
	        order.setUser(user);
	        order.setRazorpayOrderId("TEST_ORDER");   // dummy
	        order.setRazorpayPaymentId("TEST_PAYMENT"); // dummy
	        order.setRazorpaySignature("TEST_SIGNATURE"); // dummy
	        order.setOrderDate(LocalDateTime.now());
	        order.setStatus("PAID"); // simulate success
	        order.setDeliveryAddress(address);
	        ...
	        map.put("success", "Order placed successfully (TEST MODE)");

	        return "redirect:/user/orders";

	    } catch (Exception e) {
	        map.put("error", "Something went wrong: " + e.getMessage());
	        return "redirect:/checkout";
	    }
	}
